...
module.exports = function socket (socket) {
      ...
      socket.on('control', function socketOnControl (controlData) {
        switch (controlData) {
          case 'replayCredentials':
            stream.write(socket.request.session.ssh.password + '\n')
          /* falls through */
          default:
  ...
  conn.on('keyboard-interactive', function connOnKeyboardInteractive (name, instructions, instructionsLang, prompts, finish) {
    ...
    finish([socket.request.session.ssh.password])
  })
  if (socket.request.session.ssh && socket.request.session.ssh.user && socket.request.session.ssh.password) {
    conn.connect({
